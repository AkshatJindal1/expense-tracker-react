...
const HomePage = ({ user, transactions, accounts, onNavigate }) => {
  ...
  const dashboardSummary = useMemo(() => {
    ...
    const monthTx = transactions.filter(t => t.date && t.date.toDate() >= monthStart);
        ...
        <div onClick={() => setExpandedSummary(expandedSummary === 'Income' ? null : 'Income')}>
            <div className="summary-row flex items-center justify-between p-2 rounded-md hover:bg-gray-100 cursor-pointer">
                ...
                <div className="font-medium text-green-600">
                    {dashboardSummary.income.toLocaleString("en-IN", { style: "currency", currency: "INR" })}
                </div>
            </div>
            {expandedSummary === 'Income' && <div className="text-sm text-gray-600 pl-12 pt-2">Details for this month's income.</div>}
        </div>
        {/* Spending Row */}
         <div onClick={() => setExpandedSummary(expandedSummary === 'Spending' ? null : 'Spending')}>
            <div className="summary-row flex items-center justify-between p-2 rounded-md hover:bg-gray-100 cursor-pointer">
                ...
                <div className="font-medium text-red-600">
                    {dashboardSummary.spending.toLocaleString("en-IN", { style: "currency", currency: "INR" })}
                </div>
            </div>
            {expandedSummary === 'Spending' && <div className="text-sm text-gray-600 pl-12 pt-2">Details for this month's spending.</div>}
        </div>
      </div>
